...
export class EndlessMaze{
 ...
 update(player,dt,time,yaw=0,active=true,eyeHeight=1.65){const changed=this.model.update(player,yaw);if(changed.added.length||changed.removed.length||changed.mutated!==null||changed.rebased||changed.expired!==null)this.sync();const sample=this.model.sample(player.x,player.z);const sight=sample.chunk?this.secondLook.update({room:sample.chunk.room,seed:this.model.seed,x:sample.x,z:sample.z,yaw,active,dt}):null;if(sight?.cue)changed.secondLookCue={x:this.model.offset.x+sight.x,z:this.model.worldZ(sight.index)+sight.z};if(this.watcher){this.watcher.visible=!!sight&&sight.opacity>0;if(sight){this.watcher.position.set(this.model.offset.x+sight.x,0,this.model.worldZ(sight.index)+sight.z);for(const material of this.watcherMaterials)material.opacity=sight.opacity;}}const near=[];for(const[i,g]of this.groups){const c=this.model.chunks.get(i);if(active)g.userData.recognition?.update(time);if(g.userData.ghost){const ghost=g.userData.ghost,p=this.model.sample(player.x,player.z);if(p.index===i&&enteredGhostRoom(c.room,p.x,p.z))c.ghostGone=true;ghost.visible=!c.ghostGone;
