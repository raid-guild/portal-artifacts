...
import {WallWear} from './wall-wear.js';
import {Elevator} from './elevator.js';
import {meetingFurniture,galleryPosters,preparePortrait} from './meeting.js';
import {nurseryFurniture} from './nursery.js';
import {cameraPositions,trackAngle,cameraBlink} from './surveillance.js';
import {SecondLook,secondLookEligible,lookBackPoster} from './second-look.js';
...
export class EndlessMaze{
 ...
 build(chunk){if(chunk.room.kind==='recognition'){const recognition=new Recognition(this.props),g=recognition.root;g.userData={recognition,token:chunk.token,doors:[{id:'observation',pivot:recognition.pivot}],fixtures:[],glows:[],cameras:[]};return g;}const r=chunk.room,raw=new THREE.Group(),doorGroup=new THREE.Group(),fixtures=[],doors=[],cameras=[];let ghost=null;
 ...
 if(!['crawl','nursery','gallery','meeting','elevator'].includes(r.kind)){add(r.poster,r.w/2-.09,-1,1, -Math.PI/2);add('Portrait',-r.w/4,-r.d/2+.08,.9);}
 if(r.kind!=='elevator')add('DirectionPoster',r.turn+(r.turn>0?1.92:.08),-18,1,r.turn>0?-Math.PI/2:Math.PI/2);
 if(r.kind!=='elevator'&&lookBackPoster(r,this.model.seed))add('NeverLookBackPoster',r.turn+.08,-30,1.55,Math.PI/2);
 ...
 const batch=this.batch(raw);if(this.wear)batch.add(this.wear.build(chunk,this.model.seed,this.floor));if(this.floor===-1&&r.index%3!==1)for(const glow of batch.userData.glows)if(glow.material.emissive.getHex())glow.material.emissive.set(0xff3322);batch.add(doorGroup);if(ghost)batch.add(ghost);for(const c of cameras)batch.add(c.yawPivot);if(r.kind==='elevator'){const elevator=new Elevator(this.props,r.arrival);batch.add(elevator.root);batch.userData.elevator=elevator;}batch.userData={elevator:batch.userData.elevator,ghost,cameras,glows:batch.userData.glows,token:chunk.token,doors,fixtures};return batch;}
 sync(){for(const[i,g]of this.groups)if((this.entranceAttached&&i<0)||!this.model.chunks.has(i)||this.model.chunks.get(i).token!==g.userData.token){this.root.remove(g);g.userData.recognition?.dispose();g.userData.elevator?.dispose();g.userData.ghost?.userData.material.dispose();for(const c of g.userData.cameras||[])c.ledMaterial.dispose();g.traverse(o=>{if(o.isInstancedMesh){o.dispose();if(o.material.userData.mazeOwned)o.material.dispose();}});this.groups.delete(i);}for(const[i,c]of this.model.chunks){if(this.entranceAttached&&i<0)continue;if(!this.groups.has(i)){const g=this.build(c);this.groups.set(i,g);this.root.add(g);}const g=this.groups.get(i);g.rotation.y=c.room.reverse?Math.PI:0;g.position.set(this.model.offset.x,0,this.model.worldZ(i)-(c.room.reverse?52:0));}}
 update(player,dt,time,yaw=0,active=true,eyeHeight=1.65){const changed=this.model.update(player,yaw);if(changed.added.length||changed.removed.length||changed.mutated!==null||changed.rebased||changed.expired!==null)this.sync();const sample=this.model.sample(player.x,player.z);const sight=sample.chunk?this.secondLook.update({room:sample.chunk.room,seed:this.model.seed,x:sample.x,z:sample.z,yaw,active,dt,progress:this.model.facilities}):null;if(sight?.cue)changed.secondLookCue={x:this.model.offset.x+sight.x,z:this.model.worldZ(sight.index)+sight.z};if(this.watcher){this.watcher.visible=!!sight&&sight.opacity>0;if(sight){this.watcher.position.set(this.model.offset.x+sight.x,0,this.model.worldZ(sight.index)+sight.z);for(const material of this.watcherMaterials)material.opacity=sight.opacity;}}const near=[];for(const[i,g]of this.groups){const c=this.model.chunks.get(i);const lift=g.userData.elevator;if(lift?.state.arrival&&lift.state.phase==='open'&&(sample.index!==i||Math.hypot(sample.x,sample.z)>3.8))lift.state.retire();if(lift?.state.phase==='retiring'&&i!==this.model.current)lift.update(dt);g.userData.recognition?.update(time,active&&sample.index===i,!!c.doors.find(d=>d.id==='observation')?.open);if(g.userData.ghost){const ghost=g.userData.ghost,p=this.model.sample(player.x,player.z);if(p.index===i&&enteredGhostRoom(c.room,p.x,p.z))c.ghostGone=true;ghost.visible=!c.ghostGone;
