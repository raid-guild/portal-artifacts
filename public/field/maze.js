...
export class EndlessMaze{
 ...
 build(chunk){const r=chunk.room,raw=new THREE.Group(),doorGroup=new THREE.Group(),fixtures=[],doors=[];let ghost=null;
 ...
 if((x%3===1&&z%3===1&&z>=-6)||(x===r.turn&&z%7===0)||(x===0&&z===-31)||(r.branch&&x===branchX(r)&&z%7===0)||(r.branch&&z===-39&&x%7===0)){const suspended=r.hanging&&height>3,drop=suspended?height-3.5:0;
 ...
 dispose(){this.scene.remove(this.root);for(const g of this.groups.values()){g.userData.ghost?.userData.material.dispose();g.traverse(o=>{if(o.isInstancedMesh){o.dispose();if(o.material.userData.mazeOwned)o.material.dispose();}});}for(const l of this.lamps){this.scene.remove(l);l.dispose();}this.groups.clear();}
}
