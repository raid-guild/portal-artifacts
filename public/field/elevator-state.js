...
export class ElevatorState{
 ...
 prompt(x,z){if(this.phase==='closed'&&Math.abs(x)<2.3&&z>1.6&&z<4)return 'Call elevator';if(this.phase==='open'&&insideCab(x,z)&&!this.arrival)return 'Close doors · Descend';return null;}
 interact(x,z){if(!this.prompt(x,z))return false;this.phase=this.phase==='closed'?'opening':'closing';this.time=0;return true;}
 ...
 get locked(){return ['closing','travel','departed','arrival'].includes(this.phase);}
