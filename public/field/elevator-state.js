...
// Accept the doorway or the actual interior control panel, including looking down at it.
export const facesLiftDoors=(yaw,pitch=0,x=0,z=0,eyeHeight=1.65)=>{
 const dx=-x,dz=ELEVATOR_REVEAL_Z-z;
 const doorway=(-Math.sin(yaw)*dx-Math.cos(yaw)*dz)/Math.hypot(dx,dz)>.97&&Math.abs(pitch)<.45;
 const px=1.23-x,py=1.2-eyeHeight,pz=1.37-z;
 const panel=(-Math.sin(yaw)*Math.cos(pitch)*px-Math.sin(pitch)*py-Math.cos(yaw)*Math.cos(pitch)*pz)/Math.hypot(px,py,pz)>.8;
 return doorway||panel;
};
...
export const insideCab=(x,z)=>Math.abs(x)<=1.24&&z>= -1.24&&z<=1.2;
export class ElevatorState{
 ...
 retire(){if(this.arrival&&this.phase==='open'){this.phase='retiring';this.time=0;}}
 prompt(x,z,facingDoor=true){if(this.phase==='closed'&&!this.arrival&&Math.abs(x)<2.3&&z>1.6&&z<4)return 'Call elevator';if(this.phase==='open'&&insideCab(x,z)&&!this.arrival&&facingDoor)return 'Close doors · Descend';return null;}
 ...
 if(this.phase==='retiring'){this.open=Math.max(0,this.open-dt*.65);if(this.open===0)this.phase='sealed';}
