...
export function recipe(index,seed,revision=0){
 ...
 if(revision){const base=recipe(index,seed,0).kind;kind=['crawl','stairs','pit'].includes(base)?base:['empty','desk','chairs','archive','gallery'][Math.floor(r()*5)];}
...
function obstacles(room){if(room.kind==='desk')return[{x:0,z:0,w:1.8,d:1}];if(room.kind==='chairs')return[{x:room.w/2-1.7,z:0,w:1.5,d:2.2}];if(room.kind==='archive')return[{x:-room.w/2+1,z:0,w:1.4,d:4}];return[];}
export class MazeTopology{
 ...
 update(player){const index=this.indexAt(player.z);if(index!==this.current)this.pendingExpiry={index:this.current,direction:Math.sign(index-this.current)};const delta=this.ensure(index);let rebased=0;if(Math.abs(index-this.origin)>128){rebased=(index-this.origin)*SPAN;this.origin=index;player.z+=rebased;}
