...
export class MazeTopology{
 constructor(seed=Date.now(),offset={x:0,z:0},floor=null){this.floor=floor;this.offset=offset;this.seed=seed>>>0;this.origin=0;this.current=0;this.chunks=new Map();this.milestones=new Map();this.history=new Map();this.serial=0;this.discovered=0;this.facilities=0;this.mutations=0;this.maxDepth=0;this.minVisited=0;this.maxVisited=0;this.minMade=Infinity;this.maxMade=-Infinity;this.ensure(0);}
 worldZ(index){return this.offset.z+(this.origin-index)*SPAN;}
 indexAt(z){return this.origin+Math.floor((6-(z-this.offset.z))/SPAN);}
 make(index,revision=0){const room=recipe(index,this.seed,revision);room.floor=this.floor;if(this.floor===-1)room.ghost=rng(this.seed^Math.imul(index+71,2246822519)^Math.imul(revision+1,3266489917))()<.8;if((this.floor===1&&index===13)||(this.floor===-1&&index===0&&revision===0))Object.assign(room,{kind:"elevator",arrival:this.floor===-1,w:12,d:12,branch:false,narrow:false,deadEnd:false,ceiling:3});if(this.floor===-1&&index===13)Object.assign(room,{kind:"recognition",w:12,d:12,branch:false,narrow:false,deadEnd:false,ceiling:3.4});if(this.milestones.has(index))Object.assign(room,{kind:this.floor===1?"elevator":"recognition",arrival:false,reverse:this.milestones.get(index),w:12,d:12,branch:false,narrow:false,deadEnd:false,ceiling:this.floor===1?3:3.4});return{room,cells:roomCells(room),obstacles:[...obstacles(room),...(room.branch?[{x:branchX(room)+2.8,z:-22,w:.8,d:2.8}]:[])],doors:roomDoors(room),doorOpen:room.kind==='crawl',changed:false,token:++this.serial};}
 ...
 for(const i of this.chunks.keys())if(Math.abs(i-index)>1){const old=this.chunks.get(i).room;this.history.delete(i);this.history.set(i,{revision:old.revision,turn:old.turn});while(this.history.size>64)this.history.delete(this.history.keys().next().value);this.chunks.delete(i);removed.push(i);}if(index!==this.current){this.discovered++;this.maxDepth=Math.max(this.maxDepth,Math.abs(index));}this.minVisited=Math.min(this.minVisited,index);this.maxVisited=Math.max(this.maxVisited,index);this.current=index;const entered=this.chunks.get(index);if(!entered.visited){entered.visited=true;this.facilities++;}return{added,removed};}
 sample(x,z){const index=this.indexAt(z),chunk=this.chunks.get(index);const lx=x-this.offset.x,lz=z-this.worldZ(index),reverse=!!chunk?.room.reverse;return{chunk,index,x:reverse?-lx:lx,z:reverse?-52-lz:lz,reverse};}
 ...
 if(s.chunk&&!s.chunk.changed&&!['elevator','recognition'].includes(s.chunk.room.kind)&&!s.chunk.room.deadEnd&&s.z< -15&&s.z> -24&&Math.abs(s.x-(s.chunk.room.turn+1))<.85){const old=s.chunk;const next=this.make(index,old.room.revision+1);next.room.turn=old.room.turn;next.visited=old.visited;next.ghostGone=old.ghostGone;this.refresh(next);next.changed=true;next.doorOpen=old.doorOpen;this.chunks.set(index,next);this.mutations++;mutated=index;}
 ...
 // Both neighboring sections are hidden beyond bends here. After 13 facilities,
 // either direction leads to the floor milestone, including on older saved surveys.
 if(this.floor!==null&&!this.milestones.size&&this.summary().facilities>=13&&s.chunk&&!['elevator','recognition'].includes(s.chunk.room.kind)&&s.z< -16&&s.z> -28&&(Math.abs(s.x-(s.chunk.room.turn+1))<.85||(s.chunk.room.branch&&Math.abs(s.x-(branchX(s.chunk.room)+1))<.85))){
  for(const direction of [-1,1]){const target=index+direction;this.milestones.set(target,direction<0);this.chunks.set(target,this.make(target));delta.added.push(target);}
  this.pendingExpiry=null;
 }
 return{...delta,mutated,rebased,expired};}
 summary(){return{facilities:this.facilities,roomsExplored:this.discovered+1,depth:this.maxDepth,activeRooms:this.chunks.size,mutations:this.mutations,current:this.current,kind:this.chunks.get(this.current)?.room.kind};}
}
