...
## Meeting rooms and expanded gallery

Facility 5 introduces an empty meeting room; seeded variants can recur later.
The 10 × 10 m room contains a walnut conference table, eight office chairs,
scattered meeting papers and one coffee cup. `meetingFurniture()` shares the
render layout with collision, leaving aisles around the table and chairs.
Facility 9 remains reserved for the nursery.

The gallery retains its original pictures and adds the supplied EXCELLENCE
portrait plus three CONTINUITY posters: observation, attendance and legacy.
All new wall art uses the existing solid-wall placement checks.

GOD → **Meeting room** or **Gallery** locates these rooms. The assets and mesh
poster typography are generated by `tools/create_meeting.py`; the source image
is preserved in `references/excellence-portrait.png` and packed into the GLB.
The first supplied meeting image is a mood reference; no people are added.

Run `node tests/meeting.test.mjs` for facility gating, recurring room selection,
furniture collision, walkable aisles and gallery wall support checks.
