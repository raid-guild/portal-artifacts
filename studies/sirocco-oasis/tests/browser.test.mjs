...
try{
 ...
 await page.locator('#room-view').click();expect(await page.evaluate(()=>window.__oasis.getStats().transitioning)).toBe(false);await expect(page.locator('#operator-panel')).not.toHaveAttribute('open');await page.locator('#operator-panel>summary').click();
 ...
 for(const [index,title,file] of [[1,'Pump Man','pump-man.mp3'],[2,'Mesa Jam','mesa-jam.mp3'],[3,'Oasis Vibes','oasis-vibes.mp3']]){
  await page.locator('#radio-channel').focus();await page.locator('#radio-channel').press('Home');for(let i=1;i<index;i++)await page.locator('#radio-channel').press('ArrowRight');
  await expect(page.locator('#radio-status')).toHaveText(`CH 0${index} · ${title}`);
  await expect.poll(()=>page.locator('#field-radio-audio').evaluate((a,file)=>!a.paused&&a.readyState>=2&&a.currentSrc.endsWith('/audio/radio/'+file),file),{timeout:20000}).toBe(true);
 }
 await page.locator('#deck-view').click();expect(await page.evaluate(()=>window.__oasis.getStats().transitioning)).toBe(false);await expect(page.locator('#ship-radio')).toBeHidden();await expect.poll(()=>page.locator('#field-radio-audio').evaluate(a=>a.paused)).toBe(true);
 ...
 console.log('PASS: saved quota and paused fill restored, dispatch gate, all three oasis songs play in room, silent outside, silent on reload, no browser or shader errors.');
