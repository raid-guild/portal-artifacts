...
export function createControlRoom({root,model,state,act,setRate}){
 ...
 return {room,radio,hit,setView(v,transition=false){view=v;room.visible=v==='room';root.traverse(o=>{if(o.userData.cabinShell)o.visible=v!=='room'&&!transition;});radio.setInside(v==='room');},pose(lookX=0,lookY=0){root.updateMatrixWorld(true);const mobile=innerWidth<700;return {position:room.localToWorld(new THREE.Vector3(0,1.46,mobile?3.1:2.1)),target:room.localToWorld(new THREE.Vector3(lookX,(mobile?1.18:1.10)+lookY,0))};},update(){
