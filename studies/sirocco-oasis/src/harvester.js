...
export function createHarvester({scene,hull,balloon,roomModel,emitWave,reduced}){
 ...
 function act(step){if(step!==state.step)return;state.toggle();save();}
 ...
 return {root,state,cockpit,update(dt,time,airship){
  for(const f of state.tick(dt))scene.remove(f.mesh);attachFlights();if(time-lastSave>5||state.step!==priorStep||state.delivered!==priorDelivered){save();lastSave=time;priorStep=state.step;priorDelivered=state.delivered;}cockpit.update();
