...
function stylize(root){
 ...
 root.traverse(o=>{if(!o.isMesh)return;const old=o.material,key=old.name;if(!mats.has(key)){const m=new THREE.MeshToonMaterial({color:palette[key]||old.color,gradientMap:ramp,side:THREE.DoubleSide});m.name=key;if(/windows/.test(key)){m.emissive=new THREE.Color('#9dffe0');m.emissiveIntensity=.5;}mats.set(key,m);}if(!parts.has(key))parts.set(key,[]);parts.get(key).push(o.geometry.clone().applyMatrix4(o.matrixWorld));});
