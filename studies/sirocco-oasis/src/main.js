...
ready=true;document.querySelector('#loading').classList.add('done');document.querySelector('#loading').setAttribute('aria-hidden','true');window.__oasis={getStats:()=>({drifting:tour,transitioning:!!viewTransition,view:roomView?'room':shipView?'ship':'vista',machinery:harvester.sound.stats(),ready,ripples:rippleCount,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,airship:ship.position.toArray(),camera:camera.position.toArray(),target:controls.target.toArray(),time:clockTime}),waterPoint:()=>{const p=new THREE.Vector3(0,.08,0).project(camera);return{x:(p.x+1)/2*innerWidth,y:(1-p.y)/2*innerHeight};}};
...
let driftElapsed=0,driftOrigin=null,roomLookX=0,roomLookY=0;
function setTour(v){if(v&&!tour){driftElapsed=0;driftOrigin=null;}tour=v;document.querySelector('#tour').setAttribute('aria-pressed',String(v));if(v)document.body.classList.add('exploring');}
document.querySelector('#tour').onclick=()=>{setTour(!tour);toast(tour?(roomView?'Watching the lake from the wheelhouse':shipView?'Drifting around Morrow':'Drifting through the expanse'):'The view is yours');};
function updateDrift(dt){
 if(!tour||viewTransition)return;
 if(!driftOrigin){driftOrigin={target:controls.target.clone(),orbit:new THREE.Spherical().setFromVector3(camera.position.clone().sub(controls.target))};}
 driftElapsed+=dt;
 if(roomView)return;
 const orbit=driftOrigin.orbit.clone();
 orbit.theta=shipView?orbit.theta+driftElapsed*.055:THREE.MathUtils.clamp(orbit.theta+Math.sin(driftElapsed*.045)*.22,-.65,.65);
 orbit.phi=THREE.MathUtils.clamp(orbit.phi+Math.sin(driftElapsed*.065)*.018,controls.minPolarAngle,controls.maxPolarAngle);
 controls.target.copy(driftOrigin.target);camera.position.copy(controls.target).add(new THREE.Vector3().setFromSpherical(orbit));camera.lookAt(controls.target);
}
...
function setShipView(v,immediate=false,cabin=false){
 if(v&&!harvester)return;const wasRoom=roomView;roomLookX=0;roomLookY=0;const cameraFrom=camera.position.clone(),targetFrom=controls.target.clone();shipView=v;roomView=v&&cabin;setTour(false);document.body.classList.toggle('room-view',roomView);document.querySelector('#ship-radio').hidden=!roomView;document.querySelector('#room-view').setAttribute('aria-pressed',String(roomView));document.querySelector('#deck-view').setAttribute('aria-pressed',String(!roomView));harvester?.cockpit.setView(roomView?'room':v?'ship':'vista',true);document.querySelector('#operator-panel').open=!roomView;document.querySelector('#work-order').open=true;document.body.classList.toggle('ship-view',v);document.querySelector('#harvest-panel').hidden=!v;document.querySelector('#ship-view').setAttribute('aria-pressed',String(v));document.querySelector('#ship-view').innerHTML=v?'<span>↗</span> Vista':'<span>⚓</span> Ship view';
...
if(!reduced){clouds.forEach(({mesh,x,phase})=>{mesh.position.x=x+Math.sin(shared.windTime.value*.012+phase)*7;mesh.quaternion.copy(camera.quaternion);});}if(harvester)harvester.update(dt,t,ship);updateDrift(dt);if(roomView&&!viewTransition&&harvester){roomLookX=THREE.MathUtils.damp(roomLookX,tour?Math.sin(driftElapsed*.23)*.19:0,3,dt);roomLookY=THREE.MathUtils.damp(roomLookY,tour?Math.sin(driftElapsed*.17)*.045:0,3,dt);const pose=harvester.cockpit.pose(roomLookX,roomLookY);camera.position.copy(pose.position);controls.target.copy(pose.target);camera.lookAt(pose.target);}if(viewTransition){const v=viewTransition;v.age+=dt;const p=v.duration?Math.min(1,v.age/v.duration):1;const ease=p*p*(3-2*p);camera.position.lerpVectors(v.from,v.to,ease);controls.target.lerpVectors(v.targetFrom,v.target,ease);camera.lookAt(controls.target);if(p===1){viewTransition=null;harvester?.cockpit.setView(roomView?'room':shipView?'ship':'vista');controls.enabled=!roomView;if(!roomView)controls.update();}}else if(!roomView&&!tour)controls.update();if(ready&&!hint.classList.contains('dismiss')){hintPosition.set(0,.08,0).project(camera);hint.style.left=((hintPosition.x+1)*innerWidth/2)+'px';hint.style.top=((1-hintPosition.y)*innerHeight/2)+'px';}splashes.update(clockTime);renderer.render(scene,camera);atmosphere.renderGrain(renderer);}
