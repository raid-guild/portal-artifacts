...
function stylize(root){
 ...
 root.traverse(o=>{if(!o.isMesh)return;const old=o.material,key=old.name;if(!mats.has(key)){const m=new THREE.MeshToonMaterial({color:palette[key]||old.color,gradientMap:ramp,side:THREE.DoubleSide});m.name=key;if(/windows|luminous/.test(key)){m.emissive=new THREE.Color('#9dffe0');m.emissiveIntensity=.5;}mats.set(key,m);}if(!parts.has(key))parts.set(key,[]);const geometry=o.geometry.clone().applyMatrix4(o.matrixWorld);for(const attribute of Object.keys(geometry.attributes)){if(!['position','normal'].includes(attribute))geometry.deleteAttribute(attribute);}parts.get(key).push(geometry);});
 const optimized=new THREE.Group();for(const [key,geos] of parts){const mesh=new THREE.Mesh(mergeGeometries(geos,false),mats.get(key));mesh.name=key;mesh.castShadow=true;mesh.receiveShadow=!key.startsWith('Airship');optimized.add(mesh);}return optimized;
}
const loader=new GLTFLoader();let loaded=0;const keys=['basin','palm-0','palm-1','palm-2','bush-0','bush-1','spire','mesa','boulder','agave','airship','harvester','water-balloon'];
const kit={};
...
let harvester,shipView=false,viewTransition=null;
...
harvester=createHarvester({scene,hull:kit.harvester,balloon:kit['water-balloon'],emitWave,reduced});document.querySelector('#ship-view').disabled=false;
...
canvas.addEventListener('pointerup',e=>{if(!down||Math.hypot(e.clientX-down.x,e.clientY-down.y)>7){down=null;return;}down=null;pointer.set(e.clientX/innerWidth*2-1,-e.clientY/innerHeight*2+1);raycaster.setFromCamera(pointer,camera);const boatHit=harvester&&raycaster.intersectObject(harvester.root,true)[0];const hit=raycaster.intersectObject(water)[0];if(boatHit&&(!hit||boatHit.distance<hit.distance)){if(!shipView)setShipView(true);return;}if(hit)rippleAt(hit.point.x,hit.point.z);});
...
canvas.addEventListener('keydown',e=>{if(e.code==='Space'){e.preventDefault();rippleAt(0,0);}if(e.code==='ArrowLeft'||e.code==='ArrowRight'){e.preventDefault();const offset=camera.position.clone().sub(controls.target);offset.applyAxisAngle(new THREE.Vector3(0,1,0),e.code==='ArrowLeft'?-.06:.06);camera.position.copy(controls.target).add(offset);controls.update();}});
...
function setShipView(v,immediate=false){
 if(v&&!harvester)return;const cameraFrom=camera.position.clone(),targetFrom=controls.target.clone();shipView=v;setTour(false);document.body.classList.toggle('ship-view',v);document.querySelector('#harvest-panel').hidden=!v;document.querySelector('#ship-view').setAttribute('aria-pressed',String(v));document.querySelector('#ship-view').innerHTML=v?'<span>↗</span> Vista':'<span>⚓</span> Ship view';
 ...
 const target=v?harvester.root.position.clone().add(new THREE.Vector3(0,innerWidth<700?-.4:.5,0)):focus.clone();const end=v?target.clone().add(new THREE.Vector3(-3.4,3.2,4.4).multiplyScalar(innerWidth<700?1.25:1)):home.clone();
 ...
 camera.position.copy(cameraFrom);controls.target.copy(targetFrom);viewTransition={from:cameraFrom,targetFrom,to:end,target,age:0,duration:immediate||reduced?0:2.3};
}
