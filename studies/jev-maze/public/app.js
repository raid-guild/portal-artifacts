...
fresh();
fetch('/api/status').then(r=>r.json()).then(s=>{serverConfigured=s.configured;currentModel=s.model;connectionStatus();}).catch(()=>{$('connection').textContent='Local server unavailable';});
$('use-key').onclick=async()=>{
  ...
  try{
    ...
    const data=await res.json();if(!res.ok)throw Error(data.error||'Key check failed.');
