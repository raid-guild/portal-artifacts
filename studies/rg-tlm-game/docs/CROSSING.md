...
The workshop is not implemented in this milestone. Stepping through the activated arch reaches a clear chapter-two completion screen, with an option to keep exploring the landing.
...
6. Return to the landing, select Transit arch, then Step through the arch. The chapter completion screen explains that the workshop is next; Keep exploring returns to the landing.

Progress saves after each selection and interaction. Refresh midway through the rings or after activation to continue. Restart explicitly clears both rooms and returns to the waystation.
...
The storage key remains `raidguild:last-mile:room-one` for compatibility; its data format is now version 2. Room identity must agree with the departed flag; active routes require the correct symbols; completed crossings require an active arch. Unknown or contradictory saves recover safely. Transient dialog state and animation phases are not saved.
...
Movement follows a safe foreground strip, without general navigation or walking through the portal. Activation crossfades to a generated image. Stepping through ends the chapter with a dialog; there is no animated vehicle transit or playable workshop yet. There is no audio.
...
Next: user playtest, then the workshop—share the notebook, meet the keeper, and complete the fictional membership ending.
